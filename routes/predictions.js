...
// ── Match consensus + suggested benchmark ─────────────────────────────────────
router.get('/match/:matchId/consensus', async (req, res) => {
  try {
    ...
    const scoreAverages = await db.get(`
      ...
      FROM predictions
      WHERE match_id = ? AND predicted_home_score IS NOT NULL AND predicted_away_score IS NOT NULL
    `, req.params.matchId);
    ...
    const getWeight = (team) => {
      const key = Object.keys(teamWeights).find((k) => String(team || '').toLowerCase().includes(k));
      return key ? teamWeights[key] : 76;
    };
    const h = getWeight(match.home_team);
    const a = getWeight(match.away_team);
    const delta = h - a;
    const rh = Math.max(0, Math.min(5, Math.round(1.25 + h / 100 + delta / 40)));
    const ra = Math.max(0, Math.min(5, Math.round(1.1 + a / 100 - delta / 42)));
    ...
    res.json({
      ...
      suggested_benchmark: {
        home_score: rh, away_score: ra,
        predicted_result: rh > ra ? 'home' : ra > rh ? 'away' : 'draw'
      }
    });
  } catch {
    res.status(500).json({ error: 'Server error.' });
...
router.get('/analytics', async (_req, res) => {
  try {
    ...
    const dailyTrend = await db.all(`
      SELECT substr(updated_at,1,10) AS day, COUNT(*) AS predictions_count,
    ...
    const outcomeTrend = await db.all(`
      SELECT predicted_result, COUNT(*) AS picks FROM predictions
      GROUP BY predicted_result ORDER BY picks DESC
    `);
    res.json({ summary, daily_trend: dailyTrend.reverse(), outcome_trend: outcomeTrend });
  } catch {
    res.status(500).json({ error: 'Server error.' });
  }
});
